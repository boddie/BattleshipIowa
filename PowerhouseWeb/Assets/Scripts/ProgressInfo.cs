...
public class ProgressInfo : MonoBehaviour 
{
    ...
    void OnGUI()
    {
        ...
        Vector2 headingDialOrigin = new Vector2(_headingRect.x + _headingRect.width / 2 - 3, _headingRect.y + _headingRect.height * 0.76f);
        Vector2 velocityDialOrigin = new Vector2(_velocityRect.x + _velocityRect.width / 2 - 3, _velocityRect.y + _velocityRect.height * 0.76f);
        float hDegree = GameController.Instance.Heading;
        float vDegree = GameController.Instance.Velocity;
        int headingX = (int)(60 * Mathf.Cos(Mathf.PI * hDegree));
        int headingY = (int)(60 * Mathf.Sin(Mathf.PI * hDegree));
        int velocityX = (int)(60 * Mathf.Cos(Mathf.PI * vDegree));
        int velocityY = (int)(60 * Mathf.Sin(Mathf.PI * vDegree));
        ...
        GUITools.DrawLine(headingDialOrigin, headingEnd, _dial, 6);
        GUITools.DrawLine(velocityDialOrigin, velocityEnd, _dial, 6);
